...
void Window::DescriptionWindow(LPCWSTR title, UINT width, UINT height)
{
	...
	if (fullScreen)
	{
		...
		//Full Screen애 맞는 디스플레이 설정으로 변경.
		//CDS : Changes Display Settings
		ChangeDisplaySettings(&dmScreenSettings, CDS_FULLSCREEN);
	}								
	else
	{
		//창모드일 경우 caption과 system menu를 추가
...
bool Window::CreateAndShowWindow(WindowContainer* pContainer)
{
	hWnd = CreateWindowEx(
		...
		NULL, // 부모 window핸들.
		NULL, // 메뉴 또는 자식 window핸들.
		this->hInstance,
		pContainer);//window생성 때 보내는 lParam
...
//윈도우 프로시저(처리기)를 새로 설정하기위해 전방선언
LRESULT CALLBACK HandlerMsgRedirect(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam);
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
		...
		case WM_NCCREATE:
		{
			...
			SetWindowLongPtr(hWnd, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(HandlerMsgRedirect));
...
int Window::GetWidth() const { return windowDesc.width; }

int Window::GetHeight() const { return windowDesc.height; }
