...
void DirectX11Graphics::Update()
{
	...
	if (Input::GetKeyboard()->IsUp('1'))
	{
		...
		if (isWireframe) deviceContext->RSSetState(rasterizerStateWireframe.Get());
...
void DirectX11Graphics::RenderFrame() // winapi의 더블버퍼링과 유사(back버퍼에 그린거를 실제 화면에 옮기는 역할) 
{
	...
	float color[4] = { 0.5f, 0.5f ,0.5f ,1.0f }; // {R,G,B,투명도}
	...
	/*Terrain*/
	terrain.Draw(cameraObj.GetComponent<DXCamera>()->GetViewProjectionMatrix(),&frustum); // 쿼드트리를 이용하여 클리핑하기 위해 절두체를 인자값으로 보냄
	/*========*/
...
bool DirectX11Graphics::InitializeDirectX(HWND hWnd, bool fullScreen)
{
	try
	{
		...
		hr = device->CreateRasterizerState(&rasterDesc, rasterizerStateWireframe.GetAddressOf());
...
bool DirectX11Graphics::InitializeScene()//VertexBuffer.h 작성후 추가
{
	try
	{
		...
		cameraObj.GetTransform()->SetPosition({ 64.0f, 11.5f, -3.0f });
		...
		//frustum(카메라 앵글 Init 및 각도 오차범위 조정)
		frustum.Initialize(cameraObj.GetComponent<DXCamera>(),-5);

		/*sprite2D*/
		DXCamera* cam = camera2D.AddComponent<DXCamera>();//Cam(Base컴포넌트)에 DXCamera(컴포넌트)추가
		cam->SetProjectionType(ProjectionType::Orthographic);//Cam의 투영방법 Set : 직교 투영
		...
		//2D이미지는 크기 조정시 w,h값을 직접 입력해줘야한다.
		sprite.GetTransform()->SetLocalScale({ 100, 100, 1 });
		/*=======*/

		/*Terrain*/
		if (!terrain.Initialize(device.Get(), deviceContext.Get(), constantMatricesBuffer, "Textures\\heightMap129.bmp", "Textures\\dirt.dds", 10))
		{
			return false;
		}
		lod.Initialize(cameraObj.GetComponent<DXCamera>());
		terrain.SetLOD(&lod);
		/*=======*/
	}
	catch (COMException& exception)
