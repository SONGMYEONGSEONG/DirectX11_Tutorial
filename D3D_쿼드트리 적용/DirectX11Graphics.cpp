...
bool DirectX11Graphics::InitializeScene()//VertexBuffer.h 작성후 추가
{
	try
	{
		...
		frustum.Initialize(cameraObj.GetComponent<DXCamera>(),-3);
		...
		/*Terrain*/
		if (terrain.Initialize(device.Get(), deviceContext.Get(), constantMatricesBuffer, "Textures\\heightmap129.bmp", "Textures\\box.jpg", 10))
			return true;
			/*=======*/
	}
	catch (COMException& exception)
